...
const LINEUP_OPEN_KEY = 'lineup-open:v1';
...
function getOpenLineups() {
  return JSON.parse(localStorage.getItem(storageKey(LINEUP_OPEN_KEY)) || '[]');
}

function saveOpenLineups(openIds) {
  localStorage.setItem(storageKey(LINEUP_OPEN_KEY), JSON.stringify([...new Set(openIds)]));
}

function isLineupOpen(gamePk) {
  return getOpenLineups().includes(String(gamePk));
}

function toggleLineupOpen(gamePk) {
  const key = String(gamePk);
  const open = new Set(getOpenLineups());
  if (open.has(key)) open.delete(key);
  else open.add(key);
  saveOpenLineups([...open]);
}
...
async function fetchGamesAndHomeRuns(date) {
  ...
  const cards = await Promise.all(games.map(async (game) => {
    ...
    let card = {
      ...
      lineup: {
        away: buildLineup(awayPlayers, activePlay?.matchup?.batter?.id),
        home: buildLineup(homePlayers, activePlay?.matchup?.batter?.id),
      },
    };
  ...
  try {
    ...
    const cards = await Promise.all(events.map(async (event) => {
      ...
      return {
        ...
        lineup: cached?.lineup || { away: [], home: [] },
      };
    }));
...
function renderLineupList(listEl, lineup, color) {
  ...
  for (const entry of lineup) {
    ...
    li.innerHTML = `
      ...
      <span class="lineup-name" title="${entry.fullName}"><span class="lineup-bat${entry.isActive ? ' active' : ''}">🦇</span> ${entry.name}</span>
...
function renderLineupCard(card, game) {
  const lineupCard = card.querySelector('.lineup-card');
  const open = isLineupOpen(game.gamePk);
  lineupCard.hidden = !open;

  setLogo(card.querySelector('.away-lineup-logo'), game.awayLogo, `${game.away} logo`);
  setLogo(card.querySelector('.home-lineup-logo'), game.homeLogo, `${game.home} logo`);
  card.querySelector('.away-lineup-team').textContent = game.away;
  card.querySelector('.home-lineup-team').textContent = game.home;
  card.querySelector('.away-lineup-team').style.color = game.awayColor;
  card.querySelector('.home-lineup-team').style.color = game.homeColor;

  renderLineupList(card.querySelector('.away-lineup-list'), game.lineup?.away || [], game.awayColor);
  renderLineupList(card.querySelector('.home-lineup-list'), game.lineup?.home || [], game.homeColor);
}
...
function bindCardInteractions(card, game) {
  ...
  card.addEventListener('auxclick', (e) => {
    ...
    e.preventDefault();
    toggleLineupOpen(card.dataset.gamePk);
    const lineupCard = card.querySelector('.lineup-card');
    lineupCard.hidden = !isLineupOpen(card.dataset.gamePk);
  });
}
