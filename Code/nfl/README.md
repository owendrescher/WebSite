...
- Scoreboard responses are client-filtered by requested season and season type. This prevents preseason games returned by an upstream multi-season payload from contaminating the Week 1 prior-year baseline.
- Week 1 win probabilities use a wider uncertainty curve than established in-season forecasts; the projected score remains visible, but offseason uncertainty pulls confidence toward 50%.
...
- Every forecast opens with a percentage key. Win lean is the model's normalized team-strength estimate, call share is the projected portion of offensive plays, and `EVENT %` is the Poisson-derived chance of at least one touchdown, sack, or interception from that player's expected event count. Scheme/success `EDGE %` values remain matchup indices centered at 50, not sportsbook probabilities. Each signal exposes its blend and raw inputs behind an **Inputs** disclosure.
- The overall view makes unit mismatches explicit with equations such as `SEA OL/RB 57 < NE DL/Run D 68`, followed by the exact line/front grades, `#/32` unit ranks, starter averages, coverage tendency, and offense performance against the opponent's primary coverage. Projected points visibly sum the historical baseline plus roster, scheme, unit, and player adjustments. A two-row base audit shows `52% × continuity-adjusted offense scoring + 48% × continuity-adjusted opponent points allowed + SOS + home field`, including the raw PF/PA, continuity percentages, and regression toward 22.0.
- A current-roster correction compares each club's projected 2026 starters with its evidence-backed 2025 lineup. Additions, departures, unavailable starters, offensive-line availability, and offense/defense continuity alter the forecast; low continuity reduces how much last season's scoring result is allowed to dictate the new team. Skill-position valuation also includes RB2, TE2, and WR4 so an important rotational departure is not omitted. A healthy five-man line receives a recovery adjustment when the prior club had to start extra linemen.
...
- Player Edges separates QB, WR + TE, RB, and Defense. The WR + TE view stacks every current WR1–WR4 and TE1–TE2 vertically by team. It carries prior target earning into the current depth role, normalizes the room to a 79% WR/TE target pool, and displays projected target share, targets, yards, expected touchdowns, 1+ touchdown probability, aDOT, defender lane, and vertical/scheme fit. The visible formulas trace prior volume × projected attempts × opponent × matchup rather than presenting unexplained percentages.
- Defensive Imbalances compares pass defense/rush against pass offense/protection and front/run defense against OL/run offense. Beneath the team comparison, individual defenders receive matchup-adjusted `1+ sack` and `1+ interception` probabilities plus expected tackles. Edge/interior rushers are aligned with the relevant tackle or interior blocker, corners follow corresponding WR depth, off-ball linebackers draw TE lanes, and safety interception expectations react to the opposing quarterback's prior interception rate. Expected sack/INT counts remain visible as secondary `xSCK`/`xINT` context, and unusually high probabilities receive a glow/highlight drawn from that player's team palette.
...
- Team styling is generated from primary, secondary, and tertiary palettes for all 32 clubs, with luminance-selected light/dark text tokens for contrast. Team-scoped forecast copy alternates a readable lightened secondary accent with the club's primary color outlined in white; for example, New England uses light red plus outlined navy while Seattle uses action green plus outlined navy.
...
- Quarterbacks use EPA per attempt, CPOE, yards per attempt, touchdown rate, interception rate, and workload. Backs use scrimmage production, yards and EPA per carry, and first-down rate. Receivers and tight ends use target earning and yards per game, while per-target efficiency is regressed by workload so a small hot sample cannot outrank an established starter. The game model target-weights the current WR/TE room and adds a bounded vertical-fit term from quarterback YPA/EPA and the room's target-weighted aDOT. Edge, interior line, linebacker, corner, and safety formulas are separate and normalize sacks, hits, TFLs, passes defended, interceptions, and tackles per 100 defensive snaps. Offensive linemen use snap share, starts, and lineup stability because public NFL box scores do not publish individual pressure-allowed totals.
- `LILB` and `RILB` depth-chart labels are normalized into the linebacker group, so listed 3–4 inside linebackers remain eligible for starter, rating, TE-coverage, run-defense, and tackle calculations.
...
3. Confirm Receiving shows WR1–WR4 against CB1–CB4, Pass Game shows LT–LG–C–RG–RT, Player Edges switches among QB/WR + TE/RB/Defense, WR + TE shows target shares and all six threats per team, Defense shows individual sack/INT/tackle projections, the scheme stage shows outclassed/competitive labels plus base front and blitz/man/zone/pressure rates, and League Ranks contains all six unmodified 2025 `#/32` rows.
