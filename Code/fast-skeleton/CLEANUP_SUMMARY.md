...
## v39 lock icon scoreboard visibility
- Added a scoreboard-specific lock-icon override so locked tossup state appears on scoreboard cards, not just the lineup header.
- Kept play-by-play marquee clipping on `.score-mini-last-play`, while allowing the `PRE`/inning chip itself to render the lock visibly.
- No JS function trimming or feature removal.


v41 scoreboard B/S/O, bases, and completed-play text fix:
- Restored visible `B`, `S`, and `O` labels after the v40 compacting pass hid them.
- Restored the base runner diamond to the proper 18px geometry so first/second/third do not drift.
- Kept the play-by-play lane wide enough for compact results like `Carpenter 1B`.
- Scoreboard play text now prefers the most recent completed play label from active/all play history and skips transient `At Bat` labels when a completed result exists.
- No feature trimming or full-card repaint changes.
