...
function lineupDashboardStatsHtml(entry, recentStatsMap = new Map()) {
  ...
  if (!recent) {
    return '<span class="lineup-dashboard-stats">Last 5 games: AB -- | H -- | AVG --- | XBH -- | HR -- | BB -- | K --</span>';
  }
  return `<span class="lineup-dashboard-stats">Last 5 games: AB ${recent.atBats} | H ${recent.hits} | AVG ${escapeHtml(recent.avg || '---')} | XBH ${recent.extraBaseHits} | HR ${recent.homeRuns} | BB ${recent.walks} | K ${recent.strikeOuts}</span>`;
}
...
function sanitizeStoredGameCard(card) {
  ...
  if (shouldPreferProbablePitcher(card)) {
    probablePitchers = {
      away: isTrustedPreviewProbable(probablePitchers.away, card) ? probablePitchers.away : null,
      home: isTrustedPreviewProbable(probablePitchers.home, card) ? probablePitchers.home : null,
    };
  }
  return {
    ...card,
...
function scheduleTeamAbbrev(team = {}) {
  return canonicalTeamAbbrev(team?.abbreviation || team?.teamCode?.toUpperCase() || team?.fileCode?.toUpperCase() || team?.name || '');
}
...
function parseProbablePitchersFromPage(rawText, awayAbbrev, homeAbbrev, targetDate = '') {
  ...
  const entries = [];
  const end = Math.min(lines.length, matchupIndex + 90);
...
function parseStartingLineupPitchersFromPage(rawText, awayAbbrev, homeAbbrev, targetDate) {
  ...
  const entries = [];
  const end = Math.min(lines.length, matchupIndex + 70);
  for (let i = matchupIndex + 1; i < end; i += 1) {
...
function parseProbablePitchersFromSavantPage(rawText, awayAbbrev, homeAbbrev, targetDate = '') {
  ...
  if (matchupIndex < 0) return null;
  const names = [];
  const end = Math.min(lines.length, matchupIndex + 80);
  for (let i = matchupIndex + 1; i < end; i += 1) {
...
async function fetchOfficialProbablePitcherForSide(teamAbbrev, opponentAbbrev, targetDate, gamePk) {
  ...
  const promise = (async () => {
    ...
    return probable ? { ...probable, source: 'statsapi-team-schedule' } : null;
  })().catch(() => {
...
async function enrichProbablePitcherFromName(pitcher, teamAbbrev) {
  ...
  return found ? { ...pitcher, ...found, source: pitcher.source || found.source } : pitcher;
}
...
async function fetchOfficialProbablePitchersForGame(game, awayAbbrev, homeAbbrev, targetDate) {
  if (shouldPreferProbablePitcher(game)) {
    ...
    return mergeProbablePitchers(startingLineupProbables || {}, pageProbables || {}, game, awayAbbrev, homeAbbrev);
  }
  ...
  return mergeProbablePitchers({ away, home }, pageProbables, game, awayAbbrev, homeAbbrev);
}
...
function teamWonScheduleGame(game, teamAbbrev) {
  const team = canonicalTeamAbbrev(teamAbbrev);
  const away = canonicalTeamAbbrev(game?.teams?.away?.team?.abbreviation || game?.teams?.away?.team?.teamCode || '');
  const home = canonicalTeamAbbrev(game?.teams?.home?.team?.abbreviation || game?.teams?.home?.team?.teamCode || '');
...
function teamWonStoredGame(game, teamAbbrev) {
  const team = canonicalTeamAbbrev(teamAbbrev);
  const away = canonicalTeamAbbrev(game?.away || game?.teams?.away?.team?.abbreviation || game?.teams?.away?.team?.teamCode || '');
  const home = canonicalTeamAbbrev(game?.home || game?.teams?.home?.team?.abbreviation || game?.teams?.home?.team?.teamCode || '');
...
function formatLastSevenRecordFromGames(games = [], teamAbbrev = '') {
  const team = canonicalTeamAbbrev(teamAbbrev);
  ...
  return wins + losses > 0 ? `L7 ${wins}-${losses}` : '';
}
...
function localTeamLastSevenRecord(teamAbbrev, date) {
  ...
  return formatLastSevenRecordFromGames([...gamesByPk.values()], teamAbbrev);
}
...
async function getTeamLastSevenRecord(teamAbbrev, date) {
  ...
  const promise = (async () => {
    ...
    let losses = 0;
    for (const game of finished) {
      const won = teamWonScheduleGame(game, team);
      if (won === true) wins += 1;
      else if (won === false) losses += 1;
    }
    return wins + losses > 0 ? `L7 ${wins}-${losses}` : standingDeltaLastSevenRecord(team, selectedDate);
  })().catch((error) => {
...
async function getLineupRecentBattingStats(playerId, game = null) {
  const id = Number(playerId);
  if (!Number.isFinite(id) || id <= 0) return null;
  const selectedDate = dateInput.value || formatDate(new Date());
  const key = `${id}:${selectedDate}:${String(game?.gamePk || 'none')}:lineup-l5-hitting`;
  if (lineupRecentBattingCache.has(key)) return lineupRecentBattingCache.get(key);
  const promise = getPlayerRecentBattingDetails(id, game)
...
async function getLineupRecentBattingStatsMap(game, lineups = []) {
  ...
  const pairs = await mapWithConcurrency(ids, 6, async (id) => {
    const stats = await getLineupRecentBattingStats(id, game).catch(() => null);
...
function pitcherNameHtml(pitcher) {
  return `${escapeHtml(pitcher?.fullName || pitcher?.name || 'Unknown')}${handednessHtml(pitcher?.throws || pitcher?.pitchHand?.code || pitcher?.pitchHand?.description)}${pitcherFireMarkerHtml(pitcher?.id)}${pitcherColdMarkerHtml(pitcher?.id)}${pitcherHomeRunRiskMarkerHtml(pitcher)}${pitcherWhipRiskMarkerHtml(pitcher)}`;
}
...
function renderHomeRunFeed(homeRuns) {
  const list = (homeRuns.length ? homeRuns : JSON.parse(localStorage.getItem(storageKey('hrs')) || '[]'))
    .slice()
    .sort((a, b) => (Number(b.eventTimeMs) || 0) - (Number(a.eventTimeMs) || 0) || Number(b.gamePk || 0) - Number(a.gamePk || 0) || Number(b.order || 0) - Number(a.order || 0));
  const seen = new Set();
  for (const hr of list) {
    ...
    if (nameEl) {
      nameEl.textContent = `${hr.batter} #${hr.jersey ?? '?'}`;
      nameEl.style.color = hr.teamColor || '#dbebff';
    }
    const metaEl = item.querySelector('.hr-meta');
    if (metaEl) {
      metaEl.textContent = `${hr.hrNo ? `${ordinalNumber(hr.hrNo)} HR of season` : 'Season HR #?'}${hr.distance ? ` | ${hr.distance} ft` : ''}${hr.eventTimeEt ? ` | ${hr.eventTimeEt} EST` : ''}`;
    }
...
async function syncLineupOverlay(game) {
  ...
  renderLineupStatusBadges(awayLineupStreakEl, game.awayStreak, game.awayLastSevenRecord || '');
  renderLineupStatusBadges(homeLineupStreakEl, game.homeStreak, game.homeLastSevenRecord || '');
  const lastSevenPromise = Promise.all([
    getTeamLastSevenRecord(game.away, officialDateForGame(game)).catch(() => ''),
    getTeamLastSevenRecord(game.home, officialDateForGame(game)).catch(() => ''),
  ]);
  ...
  if (shouldPreferProbablePitcher(game)) {
    game.pitching = emptyPitchingData();
  }
  ...
  renderLineupList(lineupOverlayEl.querySelector('.away-lineup-list'), awayDisplayLineup, game.awayColor, game.away, new Set(), new Set(), new Map(), new Map(), new Map());
  renderLineupList(lineupOverlayEl.querySelector('.home-lineup-list'), homeDisplayLineup, game.homeColor, game.home, new Set(), new Set(), new Map(), new Map(), new Map());
  ...
  try {
    ...
    const [lastSevenRecords, coldPlayerIds, hitStreakMap, batterBadgeMap, recentBattingStatsMap] = await Promise.all([
      lastSevenPromise.catch(() => ['', '']),
    ...
    const [awayLastSevenRecord, homeLastSevenRecord] = lastSevenRecords || [];
    game.awayLastSevenRecord = awayLastSevenRecord || '';
    game.homeLastSevenRecord = homeLastSevenRecord || '';
    ...
    renderLineupStatusBadges(awayLineupStreakEl, game.awayStreak, game.awayLastSevenRecord);
    renderLineupStatusBadges(homeLineupStreakEl, game.homeStreak, game.homeLastSevenRecord);
    renderLineupList(lineupOverlayEl.querySelector('.away-lineup-list'), awayDisplayLineup, game.awayColor, game.away, hotPlayerIds, coldPlayerIds, hitStreakMap, batterBadgeMap, recentBattingStatsMap);
    renderLineupList(lineupOverlayEl.querySelector('.home-lineup-list'), homeDisplayLineup, game.homeColor, game.home, hotPlayerIds, coldPlayerIds, hitStreakMap, batterBadgeMap, recentBattingStatsMap);
  } catch {}
}
...
function renderLineupList(listEl, lineup, color, teamCode = '', hotPlayerIds = new Set(), coldPlayerIds = new Set(), hitStreakMap = new Map(), batterBadgeMap = new Map(), recentStatsMap = new Map()) {
  ...
  for (let i = 0; i < lineup.length; i += 1) {
    ...
    const emojiHtml = `${indicatorHtml}${hotEmojiHtml}${coldEmojiHtml}${avgBurstHtml}${powerBurstHtml}${slugBurstHtml}${hitStreakHtml}`;
    ...
    li.innerHTML = `
      ...
      <span class="lineup-name" title="${escapeHtml(entry.fullName)}">
        <span class="lineup-name-text">${escapeHtml(entry.name)}</span>${batterHandHtml}
        ${emojiHtml}
      ...
      <span class="lineup-today">${escapeHtml(normalizeLineupTodayValue(entry.today))}</span>
    `;
...
window.addEventListener('online', () => loadGames());
