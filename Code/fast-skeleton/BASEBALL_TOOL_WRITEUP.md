...
Tracker sort modes include time added and confidence. SWAP exchanges the vertical position of Player Tracker and Home Run Feed. A persisted `hr-first` state updates DOM order, inline grid order, and an explicit rail class whose `!important` mobile rules override the original narrow-screen fallback order; either SWAP button therefore works on phones as well as desktop. Compact mode is persisted and preserves the important identity/live-line content in a shorter row.
...
Pitcher recent-HR-allowed cards use this engine for the batter who homered. The card tooltip formats the matchup line as `{batter name} ({hand}) ({Overall}{L14 Overall superscript} OVR) ({handed rating}{L14 same-hand superscript} vR/vL)`. A right-handed pitcher selects and labels the hitter's `vsRHP` as `vR`; a left-handed pitcher selects and labels `vsLHP` as `vL`. The event retains game/opponent context and pitch type/detail. Cycling a stack switches the tooltip and all ratings to the newly selected HR.

Batter recent-HR cards perform the inverse lookup. They calculate the opposing pitcher's season Overall/vs RHB/vs LHB profile from opponent SLG, opponent AVG, HR/9, K/9, WHIP, and ERA league comparisons. The batter's side selects `vsRHB` or `vsLHB`, with small-sample regression toward established Overall. Their matchup line is `{pitcher name} ({hand}) ({Overall}{L3 Overall superscript} OVR) ({handed rating}{L3 Overall superscript} vR/vL)`. Both HR directions use a three-line immediate tooltip: event timing/context, rated opponent line, then pitch detail. These HR-card tooltips intentionally bypass the normal 1.5-second sitewide hover delay.

Pitcher quality is a league-comparison system rather than a fixed-threshold score. ERA, WHIP, opponent SLG, opponent AVG, HR/9, and K/9 are each measured both by league rank and by distance from MLB's best pitcher in that facet; ERA/run prevention has the largest weight. The combined facet profile is ranked against every qualifying MLB pitcher. Workload and starter share stabilize the result and add bounded value only to already-above-average performance, so a strong full-season starter outranks the same rate line in a short or relief-only sample without allowing innings to rescue poor pitching. The top five percent of composite league profiles occupy 95–99, explicitly reserving that band for Cy Young-level seasons while keeping 100 theoretical.
...
The app fetches `rotowire-current-defaults.json` with `cache: no-store`, timestamps refresh attempts, invalidates date-specific in-memory promises, and keeps a short TTL for batting-order pages. Parsed names are validated, deduplicated, assigned slots 1-9, resolved to MLB IDs, and locked to the correct team side. A live/mirrored response must also match at least seven players on the requested club's active roster before it may replace the published fallback; a complete-looking response from the wrong team is rejected. Published RotoWire hand-specific seeds were refreshed on July 27, 2026 for all 30 clubs (60 lineups / 540 slots).
